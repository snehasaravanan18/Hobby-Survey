...
import { ArrowLeft, Users, Activity, MapPin, Clock, Brain } from "lucide-react";
...
function ChartCard({
  ...
  return (
    <Card className={`shadow-md border-primary/10 overflow-hidden${fullWidth ? " lg:col-span-2" : ""}`}>
...
export default function Results() {
      ...
      <div className="min-h-[100dvh] w-full flex items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md text-center border-destructive/20 shadow-lg">
          <CardHeader>
            ...
            <CardDescription>We could not fetch the survey data at this time.</CardDescription>
          </CardHeader>
  ...
  const { total_responses, frequency_counts, hobby_counts, top_states, free_time_counts, stress_counts } = data;
  ...
  return (
    <div className="min-h-[100dvh] w-full bg-background p-4 md:p-8 lg:p-12 relative">
      ...
      <div className="max-w-6xl mx-auto z-10 relative space-y-8">
        ...
        {/* Header */}
        <div>
          <Button asChild variant="ghost" size="sm" className="-ml-3 mb-4 text-muted-foreground hover:text-foreground" data-testid="button-back">
          ...
          <p className="text-muted-foreground mt-1">Aggregated data from all undergraduate respondents.</p>
        </div>

        {/* Total Responses Banner */}
        <div className="flex items-center gap-5 bg-primary/10 border border-primary/20 rounded-2xl px-6 py-5" data-testid="stat-total-responses">
          <div className="p-3 bg-primary/15 rounded-xl">
            <Users className="w-7 h-7 text-primary" />
          </div>
          <div>
            <p className="text-sm font-semibold uppercase tracking-widest text-primary/80">Total Responses</p>
        ...
        {total_responses === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
            ...
            {/* Popular Hobbies — full width, horizontal bar */}
            <ChartCard
              fullWidth
              title="Popular Hobbies"
              description="Most frequently selected hobbies among respondents, sorted descending"
                  ...
                  >
                    ...
                    <Bar dataKey="count" radius={[0, 6, 6, 0]} barSize={28}>
                      ...
                      {sortedHobbies.map((_, i) => (
                        <Cell key={i} fill={i === 0 ? PRIMARY : i === 1 ? PRIMARY_LIGHT : PRIMARY_LIGHTER} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
            ...
            <ChartCard
              ...
              description="How often students engage in their hobbies"
              icon={<Activity className="w-5 h-5" />}
            >
              <div className="h-[300px] w-full">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    ...
                    margin={{ top: 20, right: 16, left: -16, bottom: 24 }}
                  >
            ...
            {/* Top States — full width, horizontal bar */}
            <ChartCard
              fullWidth
              title="Top 10 States"
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <Bar dataKey="count" radius={[0, 6, 6, 0]} barSize={24}>
                      ...
                      {top_states.map((_, i) => (
                        <Cell key={i} fill={i === 0 ? PRIMARY : i <= 2 ? PRIMARY_LIGHT : PRIMARY_LIGHTER} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
